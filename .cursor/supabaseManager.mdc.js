/**
 * Gestor de Supabase para TricycleCRM
 * 
...
const dbManager = require('./dbManager.mdc.js');
  ...
  if (changes.createTables) {
    for (const tableName of changes.createTables) {
      if (dbManager.dbSchema.tables[tableName]) {
        ...
        sqlScript += generateCreateTableSQL(tableName, dbManager.dbSchema.tables[tableName]);
        sqlScript += '\n';
      } else {
  ...
  if (changes.alterTables) {
    for (const [tableName, columns] of Object.entries(changes.alterTables)) {
      if (dbManager.dbSchema.tables[tableName]) {
        ...
        sqlScript += generateAlterTableSQL(tableName, dbManager.dbSchema.tables[tableName].columns, columns);
        sqlScript += '\n';
      } else {
...
function compareDatabaseSchemas(currentSchema) {
  if (!currentSchema) {
    ...
    return {
      ...
      script: generateDatabaseScript(true)
    };
  }
  ...
  if (Object.keys(changes.alterTables).length > 0) {
    for (const [tableName, columns] of Object.entries(changes.alterTables)) {
      sqlScript += generateAlterTableSQL(tableName, currentSchema.tables[tableName].columns, columns);
      sqlScript += '\n';
    }
  }
...
// Importar funciones necesarias del gestor de base de datos
const { 
