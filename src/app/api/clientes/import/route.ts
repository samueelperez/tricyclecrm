...
export async function POST(req: Request) {
  try {
    ...
    const processBatch = async (batch: any[], operation: 'insert' | 'update') => {
      ...
      const BATCH_SIZE = 20; // Aumentamos el tamaño del lote ya que procesamos uno por uno de todos modos
      const DELAY_MS = 100; // Reducimos el tiempo de espera entre lotes
