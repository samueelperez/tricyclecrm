...
export default function SetupDatabase() {
  ...
  const handleSetupDatabase = async () => {
    setIsLoading(true);
    setStatus('loading');
    setMessage('Ejecutando migraciones en la base de datos...');
  ...
  return (
    <div className="mb-8 p-4 border rounded-md bg-white shadow-sm">
      ...
      <p className="mb-4 text-gray-600">
        {status === 'idle' ? (
          'Configura la estructura necesaria en la base de datos para gestionar las secciones visibles por usuario. Este proceso creará las tablas y vistas necesarias en Supabase.'
        ) : status === 'loading' ? (
