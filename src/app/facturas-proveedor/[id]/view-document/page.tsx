    ...
    const fetchDocumentInfo = async () => {
      try {
        ...
        const { data: facturaData, error: facturaError } = await supabase
          ...
          .select('numero_factura, archivo_nombre')
          .eq('id', id)
          .single();
        ...
        setFacturaInfo({
          numero: facturaData.numero_factura || `Factura #${id}`,
          nombre_archivo: facturaData.archivo_nombre
        });
        
        // Obtener URL del documento
        const fileExtension = facturaData.archivo_nombre?.split('.').pop() || 'pdf';
        ...
        if (urlError) {
          ...
          const altPath = `documentos/facturas-proveedor/${id}.${fileExtension}`;
  ...
  if (error || !documentUrl) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <a href="/facturas-proveedor" className="text-indigo-600 hover:text-indigo-800 flex items-center">
